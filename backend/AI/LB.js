// AI/MB.js – FINAL FIXED VERSION: 15 Chapters, Subtopics Enforced + MATH FIXES + DIAGRAM CAPTIONS
...
function parseTOC(tocContent) {
  ...
  // <--- CHANGED: Changed from 10 to 15 to force limit
...
function generateFallbackTOC(bookTopic) {
  ...
  // <--- CHANGED: Reduced fallback to 15 chapters
 const base = [
...
async function generateTOC(bookTopic, userId) {
  // <--- CHANGED: Requested EXACTLY 15 chapters in prompt
...
[... continues to Chapter 15]`;
  ...
  while (attempts < 5) {
    ...
    try {
      ...
      // <--- CHANGED: Validation check for 15 chapters
