...
async function generateTOC(bookTopic, userId) {
  const prompt = buildTOCPrompt(bookTopic, 15, 'comprehensive');
  ...
  for (let attempts = 0; attempts < 5; attempts++) {
    try {
      ...
      if (parsed.length === 15 && parsed.every(chapter => chapter.subtopics.length >= 3)) {
...
async function generateChapter(bookTopic, chapterNumber, chapterInfo, userId, chapterInfos) {
  ...
  return cleanUpAIText(await askAI(prompt, userId, bookTopic, {
    minLength: 2600,
    genOptions: { maxOutputTokens: 5000, temperature: 0.35, topP: 0.85 }
  }));
}
