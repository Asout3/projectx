// file: LB.js  (replace your existing generator file with this)
...
// === PDF Generation (HTML via Puppeteer + Sparticuz Chromium) ===
async function generatePDF(content, outputPath) {
  // This function restores the HTML->PDF flow (Prism, MathJax, styles),
  // inserts page-breaks before chapters/titles and uses sparticuz chromium safely.
  const cleaned = cleanUpAIText(content);

  // Insert explicit page-break markers before lines that look like chapter headings or top-level markdown headings.
  // This makes each chapter start on a new page when printed.
  const withPageBreaks = cleaned.replace(/\n(?=(Chapter\s+\d+)|(?=#\s))/g, '\n<div class="page-break"></div>\n');

  // Convert markdown to HTML (we include the page-break divs directly into the HTML)
  const bodyHtml = marked.parse(withPageBreaks);
  ...
  <html>
    <head>
      ...
      <style>
        ...
        html,body { background: white; color: #111; font-family: 'Inter', 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }
        body { margin: 0; padding: 0; font-size: 13.5px; line-height: 1.7; text-align: justify; }
        .cover { text-align: center; margin-top: 120px; }
        .cover h1 { font-size: 36px; margin: 0 0 0.2em; }
        .cover p { color: #cc0000; margin-top: 1.4em; }
        .page-break { page-break-before: always; width: 100%; height: 1px; }
        h1 { font-size: 24px; color: #222; margin-top: 1.2em; border-bottom: 2px solid #e0e0e0; padding-bottom: 6px; }
        h2 { font-size: 20px; color: #2c3e50; margin-top: 1em; border-bottom: 1px solid #eaeaea; padding-bottom: 4px; }
        h3 { font-size: 16px; margin-top: .8em; }
        p { margin: 0 0 1em 0; }
        ...
        .toc { background: #f7f7f7; padding: 12px; border-radius: 6px; margin-bottom: 1em; }
        .footer { font-size: 10px; text-align: center; color: #777; width: 100%; }
      </style>
    </head>
    <body>
      <div class="cover">
        <h1>Generated by Bookgen.ai</h1>
        <p>Caution: AI can make mistakes — always review.</p>
      </div>
      ...
      <script>
        // run Prism and MathJax (Prism is synchronous; MathJax will render async)
        try {
  ...
  // Launch puppeteer using sparticuz chromium binary and safe args
  let browser;
  try {
    // ensure we include no-sandbox flags as fallback
    ...
    const execPath = typeof chromium.executablePath === 'function'
      ...
      : chromium.path || undefined;

    const args = (Array.isArray(chromium.args) ? chromium.args : []).concat(extraArgs);
    ...
    // set content and wait for network / scripts to load
    await page.setContent(html, { waitUntil: 'networkidle0', timeout: 120000 });

    // ensure Prism ran (sometimes needs an evaluate to finish)
    try {
      await page.evaluate(() => {
    ...
    // Wait briefly for MathJax to render if present
    try {
      await page.waitForFunction(() => {
        // MathJax sets window.MathJax && MathJax.typesetPromise in v3
        return !(window.MathJax && typeof window.MathJax.typesetPromise === 'function');
      }, { timeout: 3000 }).catch(() => { /* ignore timeout */ });
    } catch (e) { /* ignore */ }
    ...
    await page.pdf({
      ...
      headerTemplate: `<div style="font-size:10px; text-align:center; width:100%; color:#777;">bookgenai.vercel.app</div>`,
      footerTemplate: `<div class="footer" style="width:100%; text-align:center; font-size:10px; color:#777;">Page <span class="pageNumber"></span> of <span class="totalPages"></span></div>`,
    });

    logger.info(`Generated PDF: ${outputPath}`);
  } catch (err) {
    logger.error(`PDF generation failed: ${err.message || err}`);
    throw err;
  } finally {
    try {
...
function generatePrompts(bookTopic) {
  return [
    // ... (copy your existing prompts exactly here)
    // I left this intentionally short in this snippet — paste your full prompts array.
  ];
}
