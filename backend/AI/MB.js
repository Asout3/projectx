...
async function generatePDF(content, outputPath) {
  ...
  <html lang="en">
    <head>
      ...
      <style>
        /* === CLEAN PRINT STYLES (No @page conflicts) === */
  ...
  try {
    ...
    const instructions = {
      ...
      output: {
        ...
        pdf: {
          ...
          // NATIVE HEADER/FOOTER - NO CSS CONFLICTS
          header: {
            content: '<span style="font-family: Inter; font-size: 10px; color: #6b7280;">Generated by bookgen.ai</span>',
            spacing: "5mm"
          },
          footer: {
            content: '<span style="font-family: Inter; font-size: 10px; color: #6b7280;">Page {pageNumber}</span>',
            spacing: "5mm"
          },
          waitDelay: 3000,
          printBackground: true,
    ...
    const pdfBuffer = await response.buffer();
    fs.writeFileSync(outputPath, pdfBuffer);
    logger.info(`✅ Generated premium PDF with page numbers: ${outputPath}`);
    return outputPath;
