...
// Init Google AI (HARDCODED KEY - SECURITY RISK)
...
async function askAI(prompt, userId, bookTopic) {
  ...
  // Extract system instruction and prepare conversation history
  ...
  try {
    ...
    const result = await limiter.schedule(() => model.generateContent({
      ...
      systemInstruction: {
        parts: [{ text: systemInstruction || "Your name is Hailu. You are a kind, smart teacher explaining to a curious person. Use simple, clear words, break down complex ideas step-by-step, and include human-like examples. Always start with a table of contents, then write chapters. Focus only on the requested topic, ignore unrelated contexts." }]
      }
    }));
...
async function generatePDF(content, outputPath) {
  ...
  <html lang="en">
    <head>
      ...
      <script src="https://cdn.jsdelivr.net/npm/prismjs@1.29.0/components/prism-cpp.min.js "></link>
